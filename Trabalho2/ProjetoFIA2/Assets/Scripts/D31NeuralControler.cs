...
public class D31NeuralControler : MonoBehaviour
{
    ...
    public float GetScoreBlue()
    {
        ...
        float fitness = 0;
        if (Queryable.Average(distanceToBall.AsQueryable()) < 0.05)                             // Distance to Ball
        {
            fitness += (float)(30 + 40 * (1 - Queryable.Average(distanceToBall.AsQueryable())));
        }
        else if (Queryable.Average(distanceToBall.AsQueryable()) < 0.25)
        {
            fitness += 10;
        }
        else
        {
            fitness -= 40 * Queryable.Average(distanceToBall.AsQueryable());
        }

        fitness += (float) (60 * (1 - Queryable.Average(distancefromBallToAdversaryGoal.AsQueryable())));

        if (Queryable.Average(agentSpeed.AsQueryable()) < 0.01)
        {
            fitness += (float)(1 + Queryable.Average(agentSpeed.AsQueryable()) * 100);
        }
        else if (Queryable.Average(agentSpeed.AsQueryable()) < 0.1)
        {
            fitness += (float) (5 + Queryable.Average(agentSpeed.AsQueryable()) * 10);
        }
        else if (Queryable.Average(agentSpeed.AsQueryable()) < 0.5)
        {
            fitness += (float)(10 + Queryable.Average(agentSpeed.AsQueryable()) * 12);
        }
        else
        {
            fitness += 25;
        ...
        if (Queryable.Average(ballSpeed.AsQueryable()) < 0.001)
        {
            fitness -= 1000 * Queryable.Average(ballSpeed.AsQueryable());
        }
        else if (Queryable.Average(ballSpeed.AsQueryable()) < 0.01)
        {
            fitness -= 65 * Queryable.Average(ballSpeed.AsQueryable());
        }
        else if (Queryable.Average(ballSpeed.AsQueryable()) < 0.5)
        {
            fitness -= Queryable.Average(ballSpeed.AsQueryable());
        }
        else
        {
            fitness += Math.Min((float)30, (float)(Queryable.Average(ballSpeed.AsQueryable()) * 20));
        }

        fitness += 200 * GoalsOnAdversaryGoal;
        fitness -= 300 * GoalsOnMyGoal;

        if (hitTheBall < 500) {
            fitness += (float) 0.1 * hitTheBall;
        }
        else {
            fitness += 85;
        }
    ...
    public void DebugLog(string color)
    {

        Debug.Log(color + " \ndistanceToBall: " + Queryable.Average(distanceToBall.AsQueryable())
        + "\ndistanceToMyGoal: " + Queryable.Average(distanceToMyGoal.AsQueryable())
        + "\ndistanceToAdversaryGoal: " + Queryable.Average(distanceToAdversaryGoal.AsQueryable())
        + "\ndistanceToAdversary: " + Queryable.Average(distanceToAdversary.AsQueryable())
        + "\ndistancefromBallToAdversaryGoal: " + Queryable.Average(distancefromBallToAdversaryGoal.AsQueryable())
        + "\ndistancefromBallToMyGoal: " + Queryable.Average(distancefromBallToMyGoal.AsQueryable())
        + "\ndistanceToClosestWall: " + Queryable.Average(distanceToClosestWall.AsQueryable())
        + "\nagentSpeed: " + Queryable.Average(agentSpeed.AsQueryable())
        + "\nballSpeed: " + Queryable.Average(ballSpeed.AsQueryable())
        + "\nadvSpeed: " + Queryable.Average(advSpeed.AsQueryable())

        + "\nGoalsOnAdversaryGoal: " + GoalsOnAdversaryGoal
        + "\nGoalsOnMyGoal: " + GoalsOnMyGoal
